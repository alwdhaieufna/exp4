...
class GeoMultiHeadAttention(nn.Module):
    ...
    def forward(self, input_tensor, geo_tensor, attention_mask):
        mixed_query_layer = self.query(input_tensor)
        mixed_key_layer = self.key(input_tensor)
        mixed_value_layer = self.value(input_tensor)
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer)
        attention_scores *= self.relu(1.0 - (1.0 - torch.matmul(geo_tensor, geo_tensor.permute(0, 2, 1)).unsqueeze(1)) * 10.0) * 0.5 + 0.5
...
class GeoTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        self.multi_head_attention = GeoMultiHeadAttention(
            n_heads, hidden_size, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps
        )
        self.feed_forward = FeedForward(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps)

    def forward(self, hidden_states, geo_states, attention_mask):
        attention_output = self.multi_head_attention(hidden_states, geo_states, attention_mask)
        feedforward_output = self.feed_forward(attention_output)
        return feedforward_output
...
class GeoTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        super(GeoTransformerEncoder, self).__init__()
    ...
    def forward(self, hidden_states, geo_states, attention_mask, output_all_encoded_layers=True):
        """
        Args:
        ...
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, geo_states, attention_mask)
